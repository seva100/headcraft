...
def model(w, template_ver_t, nullify_offsets_mask=None):
...
def loss_func(pred_mesh, target, src_ver, normals_target, nullify_offsets_mask, template_faces_t, device='cuda:0'):
    ...
    l_chamfer_weight = 2000.0
    pruning_dist_thr = 3.0
    pruning_normals_thr = 1.0
    l_edges_weight = 200000.0
...
def train(template_ver_t, template_faces_t, target_ver_t, target_mesh, nullify_offsets_mask,
    ...
    # --------------------------------------------------------
    # training params (hardcoded for simplicity)
    lr = 3e-2
    # --------------------------------------------------------
    ...
    for step_no in progress:
        ...
        ver = model(offsets, template_ver_t, nullify_offsets_mask=nullify_offsets_mask)
...
def process_sample(ver_path, faces_path, uv_path, gt_path, uv_mask_path, output_path, 
    ...
    # processing
    offsets, loss, loss_dict = train(template_ver_t, template_faces_t, target_ver_t, target_mesh, nullify_offsets_mask,
                                     n_iter=n_iter,
                                     visualize=visualize_progress,
                                     save_every=vis_save_every_n_steps, 
                                     save_dir=output_path,
                                     clear_save_dir=False,
                                     make_video=False, device=device, progress_bar=progress_bar)
    
    # saving npy
    np.save(os.path.join(output_path, 'offsets.npy'), offsets.cpu().data.numpy())
    ver_upd = model(offsets, template_ver_t, nullify_offsets_mask=nullify_offsets_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', type=str, 
                        help='path to the dataset folder that contains meshes in a structure "root_dir/seq_name/expr_name/subdivided/{template_decim_ver.npy, template_decim_faces.npy, template_decim_uv.npy}"')
    ...
    if args.uv_mask_path is None:
        args.uv_mask_path = os.path.join(os.path.dirname(__file__), 
                                         '..', '..', 'data', 'flame', 'flame_uv_mask_stage1_allowed_no_face.png')

    all_inp = glob(os.path.join(args.root_dir, '*', '*', 'subdivided', 'template_decim_ver.npy'))
    ...
    if args.only_n_first_expr is not None:
        ...
        for seq_name, exps in seq2available_exps.items():
            ...
            for exp_name in suitable_exps:
                all_inp.append(os.path.join(args.root_dir, f'{seq_name:03}', f'{exp_name:03}', 'subdivided', 'template_decim_ver.npy'))
    ...
    for ver_path in tqdm(all_inp):
        ...
        output_path = os.path.join(args.output_root_dir, seq_name, exp_name, 'estimated_vector_offsets')
        ...
        m_template_faces_path = os.sep.join(ver_path.split(os.sep)[:-1] + ['template_decim_faces.npy'])
        m_template_uvs_path = os.sep.join(ver_path.split(os.sep)[:-1] + ['template_decim_uv.npy'])
