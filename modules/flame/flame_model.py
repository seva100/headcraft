...
class FLAMETex(nn.Module):
    ...
    def __init__(self, config):
        ...
        # tex_mean = np.load(config.tex_mean)
        # tex_basis = np.load(config.tex_basis)
        # self.register_buffer('texture_mean', torch.from_numpy(tex_mean).float())
        # self.register_buffer('texture_basis', torch.from_numpy(tex_basis).float())
        # print("loaded")
