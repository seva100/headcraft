...
class SynthesisLayer2(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode, gain=1):
        ...
        if noise_mode == 'const':
            # print('setting noise const')
            noise = self.noise_const * self.noise_strength
        if noise_mode == 'const-none':    # introduced for a special task (custom-layout-v1)
            hor_thr = 0.585
            noise = self.noise_const * self.noise_strength
            if len(noise.shape) == 2:
                noise[:, int(hor_thr * self.resolution):] = 0
            else:
                noise[:, :, :, int(hor_thr * self.resolution):] = 0
        ...
        if noise is not None:    # corresponds to any other noise_mode
